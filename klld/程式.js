...
function doPost(e) {
  try {
    ...
    const folder = DriveApp.getFolderById('1Ej0ywGfJ20kY2p7KkiggBSRiLLc-EZvf'); // 替換為 Google Drive 資料夾 ID
    const photoUrls = [];

    data.photos.forEach((photo, index) => {
      const blob = Utilities.newBlob(
        Utilities.base64Decode(photo.split(',')[1]),
        'image/jpeg',
        `photo_${index + 1}.jpg`
      );
    ...
    const message = `照片上傳完成，共 ${photoUrls.length} 張`;
    sendLineNotify(message, photoUrls[0]); // 發送第一張圖片的 URL

    return ContentService.createTextOutput(
      JSON.stringify({ success: true, photoUrls })
...
function sendLineNotify(message, imageUrl) {
  try {
    const lineNotifyToken = PropertiesService.getScriptProperties().getProperty('line'); // 從腳本屬性中獲取 LINE Notify Token

    if (!lineNotifyToken) {
      throw new Error('LINE Notify Token 未設定');
    }
    ...
    // 構建發送參數
    const payload = {
      message: message || '通知訊息', // 如果沒有提供訊息，設定一個預設訊息
    };

    if (imageUrl) {
      payload.imageThumbnail = imageUrl; // 縮略圖 URL
      payload.imageFullsize = imageUrl; // 全尺寸圖片 URL
    }
    ...
    Logger.log('LINE Notify 錯誤: ' + error.message);
  }
}
